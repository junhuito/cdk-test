...
var isValidResourceStatus = function (status) {
...
exports.getLayerInfoByArn = (function () {
    var regex = /^arn:aws:lambda:(\w+-\w+-\w+):(\d+):layer:(.*):(\d+)$/;
    return function (arn) {
        var match = arn.match(regex);
        if (match) {
            var region = match[1], accountId = match[2], layerName = match[3], version = match[4];
            return {
                region: region,
                accountId: accountId,
                layerName: layerName,
            ...
            throw new Error("An error occurred: The provided ARN (".concat(arn, ") does not match the expected format for a lambda layer ARN."));
        }
    };
})();
var retryReqeust = function (promise, retries, when) {
    if (retries === void 0) { retries = 3; }
    if (when === void 0) { when = function (e) { return true; }; }
    return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, promise.catch(function (error) {
                    if (retries > 0) {
